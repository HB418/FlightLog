...
// Font size (rem) for the draggable hole-number labels, scaled the same
// way as the markers — 0.95rem at ICON_BASE_ZOOM, clamped so it neither
// disappears zoomed out nor takes over the screen zoomed in.
function labelFontRemForZoom(map) {
  const scale = scaleForZoom(map);
  return Math.max(0.35, Math.min(0.61 * scale, 1.6));
}
